      ...
      trigger={
        <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-white/10 hover:border-gold-400/30 bg-black/[0.04] text-sm text-ink-100 transition-colors">
          ...
          {!isHafs && typeof diffCount === "number" && diffCount > 0 && (
            <span className="ml-1 px-1.5 py-0.5 rounded-md text-[10px] bg-red-400/15 text-red-300">
              {diffCount} diff
            </span>
          )}
        </span>
    ...
    >
      <div className="space-y-2">
        <p className="text-xs uppercase tracking-wider text-ink-400 mb-1 flex items-center gap-1.5">
          <Sparkles className="h-3 w-3 text-gold-400" /> Choose a Qira&apos;ah
        </p>
        {[1, 2, 3].map((tier) => {
          const list = RIWAYAT.filter((r) => r.tier === tier);
          return (
            <div key={tier}>
              <p className="text-[10px] uppercase tracking-wider text-ink-500 mt-3 mb-1.5">
                {tier === 1 ? "Verified" : tier === 2 ? "Coming soon" : "Future readings"}
              </p>
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-ink-400 truncate" dir="rtl" lang="ar">{r.arabicName}</p>
                      </div>
                      {disabled && <span className="text-[10px] text-ink-500">soon</span>}
                    </button>
                  );
                })}
              </div>
